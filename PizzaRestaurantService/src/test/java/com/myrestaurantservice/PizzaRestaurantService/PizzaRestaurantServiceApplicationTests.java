package com.myrestaurantservice.PizzaRestaurantService;
...
class PizzaRestaurantServiceApplicationTests {
	...
	void populateDB() {
		...
		Topping riduzionePeperoniGialliEPrezzemolo = Topping.builder().name("Riduzione di Peperoni gialli e prezzemolo")
				.build();
		...
		Pizza margherita = Pizza.builder().name("Margherita").favorite(true)
				.toppings(Set.of(mozzarella, pomodoroLocale, basilico)).build();
		Pizza quattroFormaggi = Pizza.builder().name("4 Formaggi")
				.toppings(Set.of(mozzarella, gorgonzola, provolone, granaPadano)).build();
		Pizza pizzaDAsila = Pizza.builder().name("Pizzad'Asila")
				.toppings(Set.of(fiorDiLatte, patateSilane, funghiPorciniDellaSila, lardoDiMaialeNeroSilano)).build();
		Pizza pizzaSacroEProfano = Pizza.builder().name("Sacro e profano")
				.toppings(Set.of(fiorDiLatte, salsaDiPeperoniGialli, salsicciaDiFegatoSbriciolata,
						funghiPorciniDellaSila, riduzionePeperoniGialliEPrezzemolo, provoloneDelMonaco))
				.build();
		Pizza pizzaDAccarezzare = Pizza.builder().name("Pizzad'Accarezzare")
				.toppings(Set.of(fiorDiLatte, rucola, ricottaDiPecoraAlloZafferano, pomodoriSecchi, gherigliDiNoce))
				.build();
		Pizza pizzaDAlessandro = Pizza.builder().name("Pizzad'Alessandro").toppings(Set.of(fiorDiLatte, pomodoriniRossi,
				speckMaialeNeroCalabrese, granellaDiPistacchio, battutoDiPistacchioDiBronte)).build();
		Pizza pizzaEFigliata = Pizza.builder().name("Pizza e Figliata")
				.toppings(Set.of(figliata, mortadella, cremaDiGorgonzolaEPistacchio)).build();
		Pizza pizzaBufalaDiMare = Pizza.builder().name("Pizza bufala di mare").toppings(Set.of(fiorDiLatte,
				pacchetelleGialle, bottargaDiMuggine, aliciDiCetara, zesteDiLimoneDiCetraro, stracciatella, basilico))
				.build();
		pizzaRepository.saveAll(List.of(margherita, quattroFormaggi, pizzaDAsila, pizzaSacroEProfano, pizzaDAccarezzare,
				pizzaDAlessandro, pizzaEFigliata, pizzaBufalaDiMare));
		...
		Restaurant vogliaDiPizza = Restaurant.builder().name("Voglia di pizza").address("Via Roma").city("Roma")
				.drivers(Set.of(driver1, driver2, driver3)).build();
		Restaurant pizzaAGoGo = Restaurant.builder().name("Pizza a GoGo").address("Via Milano").city("Milano")
